...
def get_concentrations(I, stain_matrix, lamda=0.01):
    ...
    # numThreads=1: spams.lasso defaults to ALL cores; with many parallel workers
    # this oversubscribes the (shared) machine. 1 thread/worker -> identical result.
    return spams.lasso(OD.T, D=stain_matrix.T, mode=2, lambda1=lamda, pos=True,
                       numThreads=1).toarray().T
